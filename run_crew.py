...
if not os.environ.get("GOOGLE_API_KEY") or not os.environ.get("ANTHROPIC_API_KEY"):
    raise ValueError("API keys for Google and Anthropic must be set as environment variables.")
...
class FileSystemToolInput(BaseModel):
    operation: str = Field(description="The operation to perform. Must be one of 'write', 'read', 'list'.")
    path: str = Field(description="The relative path to the file or directory in the workspace.")
    content: Optional[str] = Field(default=None, description="The content to write to the file. Required only for the 'write' operation.")
...
class FileSystemTool(BaseTool):
    ...
    **Valid operations are 'write', 'read', and 'list'.**
    ...
    def _run(self, operation: str, path: str, content: Optional[str] = None) -> str:
        ...
        try:
            if operation == "write":
                if content is None: return "Error: The 'write' operation requires the 'content' argument."
...
class CodeTestTool(BaseTool):
    ...
    description: str = "A tool to test generated Python code by executing the file for syntax errors."
    workspace_dir: str
    ...
    def _run(self, file_path: str) -> str:
        ...
        if not os.path.exists(full_path): return f"Error: File '{full_path}' not found for testing."
        try:
            command = f"python -m py_compile {full_path}"
            result = os.system(command)
            if result == 0: return "Python syntax check passed!"
            else:
